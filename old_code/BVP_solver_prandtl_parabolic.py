...
h_BL = 40     # boundary-layer height H (m)
...
def nu(z):
    ...
    # parabolic part
    nu_par = k_karman * u_friction * z * (1.0 - z / h_BL)
    # ensure non-negative and add molecular floor
    nu_par = np.maximum(nu_par, 0.0) + molecular_viscosity
    return nu_par
...
fig.suptitle(r'Prandtl eddy-viscosity: $\nu(z)=\kappa u_* z\left(1-\frac{z}{H}\right)$', fontsize=15)
...
plt.savefig("Notes_Ekman_spiral/Figures/BVP_Prandtl1.png",dpi=400)
