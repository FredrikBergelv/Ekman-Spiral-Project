...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 24 12:46:23 2025
...
background_viscosity = 0.4   # small floor (m2/s)
...
def nu(z):
    ...
    fun_param = 0.3*(1+np.exp(-((z-40)/25)**2)*np.sin(z/2))
    nu_par = background_viscosity + k_karman*u_friction*z*np.exp(-z/30) * fun_param
...
fig.suptitle(r'Fun eddy-viscosity: $\nu(z)=\kappa u_* ze^{z/30}\cdot\left[1+\sin\left(\frac{z}{2}\right)\right]e^{-\left(\frac{z-40}{15}\right)^2}$', fontsize=15)
...
plt.savefig("Notes_Ekman_spiral/Figures/BVP_sin.png",dpi=400)
plt.savefig("Notes_Ekman_spiral/Figures/nice figures/BVP_sin.png",dpi=400)